...
| `切换工作区 [编号/路径]` / `WORKSPACE_CHANGE` | 在**已创建**的工作区之间切换 | 不带参数时列出全部工作区 |
| `切换会话 [编号]` / `CHANGE_SESSION` | 在**当前工作区**的会话之间切换 | 不带参数时列出当前工作区的会话 |
| `帮助` / `HELP` | 显示命令列表 | |
...
菜单点击后机器人先弹出**交互卡片**（每个选项一个按钮），点击按钮即可完成切换；
也可以直接在单聊里回复编号（文字命令同样支持）。
